package com.fbdblog.db.proxool;
...
public class Db {
  ...
  public static String alias = "fbdblog-"+ RandomString.randomAlphabetic(4);
  public static int configCount = 0;


    public static void setupDataSource(){
        configCount = configCount + 1;
        System.out.println("Fbdblog:"+alias+": Start proxool configuration configCount="+configCount);
        try{

            try{
                System.out.println("Fbdblog:"+alias+": Shutting down all pools.");
                ProxoolFacade.shutdown(0);
            } catch (Exception e){
                System.out.println("Fbdblog:"+alias+": Error shutting down pool.");
                e.printStackTrace();
            }
            ...
            System.out.println("Fbdblog:"+alias+": Ds setup appears successful.");
            System.out.println("Fbdblog:"+alias+": Ds url="+url);

        } catch (Exception e){
                System.out.println("Fbdblog:"+alias+": Error configuring proxool connection.");
                e.printStackTrace();
        }
        System.out.println("Fbdblog:"+alias+": End ds setup.");
    }
  ...
  public static Connection getConnection(){
        try{
            if (!driverHasBeenConfigured || InstanceProperties.haveNewConfigToTest()){
                System.out.println("Fbdblog:"+alias+": proxool driverHasBeenConfigured is false or we have a new dbconfig to test.");
                setupDataSource();
            }
            try{
                ...
                System.out.println("Fbdblog:"+alias+": Error getting connection from the dataSource ds.");
                e.printStackTrace();
            }
        } catch (Exception e){
            System.out.println("Fbdblog:"+alias+": Error setting up.");
            e.printStackTrace();
        }
        System.out.println("Fbdblog:"+alias+": Returning null connection.");
        return null;
